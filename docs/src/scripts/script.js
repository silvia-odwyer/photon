...
import("../../../crate/pkg").then(module => {
  ...
  function setUpComparisonImages() {
    ...
    wasm_canvas = document.getElementById("wasm_canvas");
    js_canvas.width = js_img.width;
    js_canvas.height = js_img.height;
    wasm_canvas.width = js_img.width;
    wasm_canvas.height = js_img.height;

    js_ctx = js_canvas.getContext("2d");
    wasm_ctx = wasm_canvas.getContext("2d");

    js_ctx.drawImage(js_img, 0, 0);
    wasm_ctx.drawImage(js_img, 0, 0);
  ...
  function compareWASM() {
    ...
    // Get the image data from the image
    let wasmImgData = wasm_ctx.getImageData(0, 0, wasm_canvas.width, wasm_canvas.height);

    // Convert the ImageData to a PhotonImage (so that it can communicate with the core Rust library)
    let rust_image = module.open_image(wasmImgData, wasm_canvas.width, wasm_canvas.height);
  ...
  // Filter the image, the PhotonImage's raw pixels are modified and 
  // the PhotonImage is returned
  let filter_name = "swap_rg_channels";
  let new_image = filter_dict[filter_name]();
  let new_pixels = module.to_image_data(new_image);
  
  // Place the pixels back on the canvas
  wasm_ctx.putImageData(new_pixels, 0, 0);
    // let imgData = ctx.getImageData(0, 0, canvas.width, canvas.height);
    // let rust_image = module.open_image(imgData, canvas.width, canvas.height);
